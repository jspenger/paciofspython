...
class Broadcast(module.Module):
    ...
    def _start(self):
        threading.Thread(target=self.deliver).start()

    def deliver(self):
        for message in iter(self.queue.get, None):
            self.northbound.upon_deliver(self.pid, self.txid, self._unpack(message))

    def broadcast(self, message):
        self.queue.put(self._pack(message))

class BlockchainBroadcast(Broadcast):
    def _createTransaction(self, privkey, pubkeyhash, prevtxhash, message):
        logger.debug('creating transaction: %s', (privkey, pubkeyhash, prevtxhash, message))
        ...
        signedtransaction = self.southbound.signrawtransaction(transaction, [], [privkey])['hex']
    ...
    def _unpackTransaction(self, txhex):
        try:
            ...
            pubkeyhash = self.southbound.decoderawtransaction(self.southbound.getrawtransaction(transaction['vin'][0]['txid']))['vout'][0]['scriptPubKey']['addresses'][0]
    ...
    def _start(self):
        if self.prevtxhash is None and self.privkey is None and self.pubkeyhash is None:
            self.privkey, self.pubkeyhash, self.prevtxhash, transaction = self.southbound._create_funded_keypair()
            self.lock.acquire()
            self.waiting[self.prevtxhash] = transaction
            self.lock.release()
        logger.info('starting blockchain broadcast: privkey=%s pubkey=%s prevtxhash=%s' % (self.privkey, self.pubkeyhash, self.prevtxhash))
    ...
    def __init__(self, prevtxhash=None, privkey=None, pubkeyhash=None):
    ...
    def _timeout_broadcast(self):
        while True:
            ...
            messages = []
            while True:
                # message = iter(self.queue.get, None)
                try:
                ...
                if t + 0.1 < time.time() or size > 100000:
                    break
            if messages != []:
                signedtransaction = self._createTransaction(self.privkey, self.pubkeyhash, self.prevtxhash, messages)
        ...
        try:
            while True:
                ...
                while bbh not in self.ledger:
                    ...
                    bbh = self.southbound.getblock(bbh, 1).get('previousblockhash', None)
                newledger.reverse()
                    ...
                    for transaction in self.southbound.getblock(block, 2)['tx']:
                        try:
                            ...
                            for message in messages:
                                logger.info('trigger deliver: pid=%s; txid=%s; message=%s' % (pid, txid, message))
...
if __name__ == '__main__':
    ...
    class FS():
        def _upon_deliver(self, pid, txid, message):
            print()
            print('received:', pid, txid, message)
            print()
        def fs_api(self, action):
            bb.broadcast(action)
    fs = FS()
    blb = BlockchainBroadcast()
    blockchain = blockchain.Blockchain()
    blockchain._create()
    blockchain._start()
    blb._register_southbound(blockchain)
    blb._register_northbound(fs)
    blb._start()
    for i in range(10):
        blb.broadcast(i)
